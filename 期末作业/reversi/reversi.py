...
class ReversiEnv(gym.Env):
    ...
    def _step(self, action):
        ...
        if color == 0:  #  黑色棋子是 0
            if ReversiEnv.pass_place(self.board_size, action):
                self.possible_actions = ReversiEnv.get_possible_actions(self.state, 1) # by xiaoguai0992
                pass
                ...
                if self.illegal_place_mode == 'raise':
                    self.raise_illegal(self.state, action) # by xiaoguai0992
                    raise
            ...
            if ReversiEnv.pass_place(self.board_size, action):
                self.possible_actions = ReversiEnv.get_possible_actions(self.state, 0 ) # by xiaoguai0992
                pass
            elif ReversiEnv.resign_place(self.board_size, action):
                return self.state, 1, True, {'state': self.state}
    ...
    def raise_illegal(self, board, action):
        print('-'*10)
                ...
                for dx in [-1, 0, 1]:
                    for dy in [-1, 0, 1]:
                        ...
                        if(n > 0 and board[player_color, nx, ny] == 1):
                            ...
                            actions.add( pos_x * 8 + pos_y ) # by xiaoguai0992

        if len(actions)==0:
            actions = [d**2 + 1]

        actions = list(actions) # by xiaoguai0992
    ...
    def valid_place(board, action, player_color):
        ...
        #print(action, coords) # by xiaoguai0992
    ...
    def game_finished(board):
        ...
        if player_score == 0:
            ...
            action_x = ReversiEnv.get_possible_actions(board, ReversiEnv.BLACK)
            action_y = ReversiEnv.get_possible_actions(board, ReversiEnv.WHITE)
            if len(action_x) == 1 and action_x[0] == (d**2)+1 \
                and len(action_y) == 1 and action_y[0] == (d**2)+1:
